...
public class ProductDaoImpl implements ProductDao{
	...
	public List<Integer> findAllCategory() {
		try {
			PreparedStatement getcat=conn.prepareStatement("select distinct cid from productSS ");
	...
	public List<Product> findByCategory(int cid) {
		try {
			PreparedStatement getprod=conn.prepareStatement("select * from productSS where cid=? ");
