...
public class ProductDaoImpl implements ProductDao {
	...
	static PreparedStatement insProd,findAllProd,findById,updateById,deleteById,sortByPrice;
	static {
		conn=DBUtil.getMyConnection();
		try {
			insProd=conn.prepareStatement("insert into myproduct values(?,?,?,?,?)");
			findAllProd=conn.prepareStatement("select * from myproduct");
	...
	public List<Product> displayAll() {
		...
		try {
			ResultSet rs = findAllProd.executeQuery();
			while(rs.next()) {
